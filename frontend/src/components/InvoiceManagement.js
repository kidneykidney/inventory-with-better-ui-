...
const InvoiceManagement = () => {
      ...
      <DialogContent dividers>
        {selectedInvoice && (
          <Grid container spacing={2}>
            ...
            {/* Acknowledgments */}
            {selectedInvoice.acknowledgments && selectedInvoice.acknowledgments.length > 0 && (
              <Grid item xs={12}>
                <Card>
                  <CardContent>
                    <Typography variant="h6" gutterBottom>
                      Student Acknowledgments ({selectedInvoice.acknowledgments.length})
                    </Typography>
                    {selectedInvoice.acknowledgments.map((ack, index) => (
                      <Box key={index} sx={{ mb: 2, p: 2, border: '1px solid #e0e0e0', borderRadius: 1 }}>
                        <Typography><strong>Type:</strong> {ack.acknowledgment_type}</Typography>
                        <Typography><strong>Method:</strong> {ack.acknowledgment_method}</Typography>
                        <Typography><strong>Date:</strong> {formatDateTime(ack.acknowledged_at)}</Typography>
                        {ack.acknowledgment_location && (
                          <Typography><strong>Location:</strong> {ack.acknowledgment_location}</Typography>
                        )}
                        {ack.witness_name && (
                          <Typography><strong>Witness:</strong> {ack.witness_name}</Typography>
                        )}
                        {ack.notes && (
                          <Typography variant="body2" sx={{ mt: 1 }}>
  ...
  return (
    <Box p={3}>
      ...
      {/* Camera Upload Dialog - This will be implemented in a separate component */}
      {/* <CameraUploadDialog /> */}
