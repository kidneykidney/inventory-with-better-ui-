-- =========================================
-- COMPLETE INVENTORY SYSTEM VIEW - ALL TABLES
-- Fresh comprehensive query for complete system overview
-- =========================================
...
SELECT 
    ...
    CONCAT('Total Inventory Value: $', COALESCE((SELECT ROUND(SUM(quantity_available * unit_price), 2) FROM products), 0)) as financial_info,
    1 as sort_order
...
SELECT 
    ...
    c.created_at::date::text as info4,
...
SELECT 
    ...
    s.name,
    CONCAT('ID: ', s.student_id) as info1,
    s.email as info2,
    COALESCE(s.department, 'No department') as info3,
    CONCAT('Year: ', COALESCE(s.year_of_study::text, 'Not set')) as info4,
    CONCAT('Orders: ', COUNT(DISTINCT o.id), 
           ' | Invoices: ', COUNT(DISTINCT i.id), 
           CASE WHEN s.is_active THEN ' | ✅ Active' ELSE ' | ❌ Inactive' END) as financial_info,
    4 as sort_order
FROM students s
...
SELECT 
    ...
    CONCAT('Issued: ', i.issue_date::date) as info4,
...
SELECT 
    ...
    CONCAT('Invoice #', i.invoice_number) as info1,
    s.name as info2,
    CONCAT('Uploaded by: ', COALESCE(ii.uploaded_by, 'System')) as info3,
...
ORDER BY sort_order, table_type, name;

-- =========================================
-- TABLE FORMAT EXPLANATION:
-- table_type: Category/Type of record
-- name: Main identifier  
-- info1: Primary details
-- info2: Secondary details
-- info3: Additional information
-- info4: Extra details
-- financial_info: Money/status information
-- =========================================
